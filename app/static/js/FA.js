...
function create_films(self) {
    ...
    var options_loading = options.loading ?
        decodeURIComponent(options.loading).trim() : 'asd'
    ...
    var language = (options.language && !/ru/i.test(options.language)) ? { "trailer": "TRAILER", "torrent": "DOWNLOAD", "next": "NEXT", "prev": "PREV" } : { "trailer": "ТРЕЙЛЕР", "torrent": "СКАЧАТЬ", "next": "ВПЕРЕД", "prev": "НАЗАД" };
    ...
    if (options.tv) {
        document.addEventListener('keydown', analys_key);
    }
    ...
    var css = '#yohoho-loadings{z-index:9999;position:absolute;left:0;top:0;width:100%;height:100%; background:#383838;}#yohoho-buttons{height: max-content;;z-index:9999;right:0;top:0px;text-align:left;display: inline-flex;text-align: left;flex-direction: row;flex-wrap: wrap;align-content: space-around;justify-content: space-evenly;align-items: flex-start;}#yohoho-buttons div{font-family:Verdana,sans-serif;font-weight:normal;text-shadow:none;line-height:normal;font-size:' /*+ 12 * options.button_size + 'px; */ + ' font-size: .75rem;color:#fff;height:max-content; background:#' + bg + ';border-radius:5px 0 0 5px;padding:5px;margin:5px 0 5px 5px;opacity:.7;}#yohoho-buttons div:hover,#yohoho-buttons div.yohoho-active{color:#fff;background:#' + bg + ';cursor:pointer;opacity:1}';
    ...
    document.getElementById('bqs').appendChild(i);
    ...
    var style = 'width:100%;border:0;margin:0;padding:0;overflow:hidden;position:relative;  margin-bottom: 15px;'; //height:inherit;
    i.setAttribute('style', style);
    i.setAttribute('width', w);
    i.setAttribute('height', h);
    yohoho.setAttribute('style', style);

    connect_server(options_url, p,
        function(players) {
            var first = true;
            var buttons = document.createElement('div');
            buttons.setAttribute('id', 'yohoho-buttons');
            buttons.setAttribute('class', 'aqsq');
            ...
            for (var i = 0, len = keys.length; i < len; i++) {
                ...
                if (players.hasOwnProperty(key) && players[key] && players[key].iframe) {
                    ...
                    option.setAttribute('onclick', 'connect_pl("' + encodeURIComponent(players[key].iframe) + '", "' + players[key].quality + '", "' + players[key].translate + '", this, "' + options.button_size + '")');
                    ...
                    if (first) {
                        ...
                        connect_pl(players[key].iframe, players[key].quality, players[key].translate, option, buttons, options.button_size);
                    ...
                    if (j && !(j % options.button_limit) && keys[i + 1]) {
                        ...
                        next.setAttribute('onclick', 'change_player(' + Math.ceil((j + 1) / options.button_limit) + ', "' + options.button_size + '");' + 'connect_pl("' + encodeURIComponent(players[keys[i + 1].toLowerCase().trim()].iframe) + '", "' + players[keys[i + 1].toLowerCase().trim()].quality + '", "' + players[keys[i + 1].toLowerCase().trim()].translate + '", document.querySelector(\'[data-event="' + (j + 1) + '"]\'), "' + options.button_size + '")');
                        ...
                        prev.setAttribute('onclick', 'change_player(' + Math.ceil(j / options.button_limit) + ', "' + options.button_size + '");' + 'connect_pl("' + encodeURIComponent(players[keys[i - 1].toLowerCase().trim()].iframe) + '", "' + players[keys[i - 1].toLowerCase().trim()].quality + '", "' + players[keys[i - 1].toLowerCase().trim()].translate + '", document.querySelector(\'[data-event="' + (j) + '"]\'), "' + options.button_size + '")');
            ...
            if (j < 1) {
                ...
                if (keys.length > options.button_limit) {
                    change_player(1, options.button_size);
                }
            }
        });
...
// Установка плееров, и подключения к стоковым плеерам
function connect_pl(iframe, quality, translate, element, buttons, size) {
    ...
    if (iframe.indexOf('4h0y') + 1) {
        ...
        connect_server(decodeURIComponent(iframe), '', function(json, html) {
...
// Обработчик окошка с выбором плееров
function change_player(page, size) {
...
// Получаем данные всех плееров json
function player_json(jsonString) {
...
// Открытие на всё окно, вариант через стили (Вроде как одаптивнее)
...
// Меняем размер блока фильма
function _resize(w = '', h = '', d = false, daq = '') {
    ...
    var w
    if (w == '') { w = parseInt(yi.parentNode.parentNode.offsetWidth); }
    if (h == '') { h = parseInt(yi.parentNode.parentNode.offsetHeight); }
    yi.style.width = w + 'px';
    yi.setAttribute('width', w.toString());
    yi.parentNode.style.width = w + 'px';
    if (window.innerWidth > 700) {
        yi.style.height = 'calc(10vw * 4)';
    } else {
        yi.style.height = 'calc(10vw * 6)';
    }

    yi.style.width = 'calc(10vw * 10)';
    yi.parentNode.style.width = 'auto';
    if (d = true) {
        yi.style.background = '#383838 url(/static/photo/Dancing-baby.dfdc1c16.gif) 50% 50% no-repeat';
        yi.setAttribute('height', 'auto');
        yi.parentNode.style.height = 'auto';
...
function _q(types_ = 'types_') {
    var yohohoPage = document.querySelector('#yohoho-iframe').querySelectorAll('div[data-page="' + page + '"]');
    //console.log(yohohoPage);
    if (yohohoPage) {
        for (var j = 0; j < yohohoPage.length; j++) {
            yohohoPage[j].style.display = 'block';
        }
